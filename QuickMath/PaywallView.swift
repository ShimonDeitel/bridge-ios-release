...
struct PaywallView: View {
    ...
    private let benefits: [(String, String, String)] = [
        ("calendar", "Daily puzzle archive", "Replay every past day's bridges puzzle, all the way back."),
        ("square.grid.3x3.fill", "Bigger boards", "Step up to larger, tougher island layouts."),
        ("brain.head.profile", "An expert puzzle daily", "A second, harder hand-made puzzle every single day."),
        ("lightbulb.fill", "Hints & themes", "Reveal a bridge when you're stuck, plus board themes.")
    ]
            ...
            ScrollView {
                VStack(spacing: 22) {
                    VStack(spacing: 8) {
                        ...
                        Text("Bridge Pro").font(.largeTitle.weight(.heavy))
                    ...
                    VStack(spacing: 12) {
                        Button { Task { await buy() } } label: {
                            HStack {
                                ...
                                Text(working ? "Unlocking…" : "Unlock Bridge Pro · \(store.displayPrice)")
                                    .font(.headline)
                            }
                        ...
                        Text("Bridge Pro is a $0.99/month subscription that renews automatically unless canceled at least 24 hours before the period ends. Payment is charged to your Apple Account; manage or cancel anytime in Settings.")
                        ...
                        Text("Bridge never tracks you. Your progress stays on your device.")
                            .font(.footnote).foregroundStyle(.secondary)
                            .multilineTextAlignment(.center).padding(.top, 4)
                    }
